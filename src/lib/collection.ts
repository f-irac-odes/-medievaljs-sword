...
export class Collection<T extends AnyEntity = any> {
	...
	query(
		...
		if (typeof filter === 'function') {
			return this._entities.map((entity) => this._withUtilities(entity)).filter(filter);
		} else {
	...
	private updateQuery(matchedEntities: Partial<T>[]) {
		const addedEntities = matchedEntities.filter((entity) => !this._entities.includes(entity));
		const removedEntities = this._entities.filter((entity) => !matchedEntities.includes(entity));
